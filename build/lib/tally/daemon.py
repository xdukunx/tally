...
from . import batch, cli, db, notifier, scheduler
...
def _parse_submit(parts: list[str]):
    """Parse `/submit --cores N [...] -- cmd args` into (specs, cmd).
    Reuses the batch directive flag parser. Raises ValueError on bad input."""
    if "--" not in parts:
        raise ValueError("missing '-- command' (use: /submit --cores N -- cmd args)")
    sep = parts.index("--")
    specs = batch.parse_spec_flags(parts[:sep])
    cmd = parts[sep + 1:]
